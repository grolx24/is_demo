...
def get_all_calls(but, now_date):
...
def get_app_tasks_id(but):
    ...
    app_tasks_id = but.call_list_method("app.option.get",
                                    {"option": "tasks"})
    return app_tasks_id
...
def set_app_possible_calls(but, calls):
    """Позволяет добавить дату в опции текущего приложения"""

    but.call_api_method("app.option.set", {"options": {"possible_calls": calls}})
